package com.valenpatel.paisefy.viewmodel
...
import com.valenpatel.paisefy.repository.Repository
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val archiveList: LiveData<List<ArchivedTransaction>>
    val recentlyDeletedTransactions: LiveData<List<RecentlyDeletedDataEntity>>
    ...
    init {
        ...
        archiveList = repository.archivedTransactions
        recentlyDeletedTransactions = repository.recentlyDeletedData
    }
...
fun getAllArchivedTransactions(): LiveData<List<ArchivedTransaction>> {
    return repository.archivedTransactions
}
     suspend fun insertArchivedTransactions(transaction: Transaction){
        return repository.archiveTransaction(transaction)
    }

    fun insertArchivedTransaction(transaction: Transaction) {
   ...
   fun getAllDeletedransactions(): LiveData<List<RecentlyDeletedDataEntity>> {
    return repository.recentlyDeletedData
}

    fun insertRecentlyDeletedTransaction(transaction: Transaction) {
        viewModelScope.launch {
            repository.recentlyDeletedTransaction(transaction)
        }
    }
